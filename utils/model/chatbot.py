...
# --- Ollama ChatBot ---
def chatbot_ollama(text, model_name = "llama3.2:1b"):
    url = OLLAMA_API_URL
    data = {
        "model": model_name,
        "messages": [
...
def chatbot_any(text, provider = "ollama", model_name = None):
    if provider.lower() == "ollama":
        return chatbot_ollama(text, model_name)
    elif provider.lower() == "gemini":
        return chatbot_gemini(text, model_name)
    else:
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python chatbot.py <provider> <model_name> <text>", file = sys.stderr)
        sys.exit(1)
